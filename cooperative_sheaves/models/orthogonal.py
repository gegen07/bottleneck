...
class Orthogonal(nn.Module):
    ...
    def forward(self, params: torch.Tensor) -> torch.Tensor:
        if self.orthogonal_map != "euler":
            ...
            #     (params.size(0), params.size(1), self.d, self.d), dtype=params.dtype, device=params.device)
        ...
        if self.orthogonal_map == "matrix_exp" or self.orthogonal_map == "cayley":
            ...
            # eye = torch.eye(self.d, device=params.device).unsqueeze(0).repeat(params.size(0), params.size(1), 1, 1)
            ...
            assert 2 <= self.d <= 3
            if self.d == 2:
                # half_b = params.size(1) // 2
                # Q1 = self.get_2d_rotation(params[:, :half_b], det=1)
                # Q2 = self.get_2d_rotation(params[:, half_b:], det=-1)
